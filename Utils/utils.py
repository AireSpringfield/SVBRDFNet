import sys, os, math, random, glob, re, io
import numpy as np
import cv2
...
def pfmFromBuffer(buffer, reverse = 1):
    ...
    header = sStream.readline().rstrip()    # delete spaces in the end
    color = (header == 'PF')    # true for 3 channel and false for gray scale
    ...
    sStream.close()
    if(len(shape) == 3):
        return rawdata.reshape(shape).astype(np.float32)[:,:,::-1]  # rgb to bgr
    else:
...
def load_pfm(filepath, reverse = 1):
    ...
    width, height = map(int, file.readline().strip().split(b' '))
...
def load_and_clip(filepath, left, top, width, height, reverse = 1):
    ...
    if(ext == '.pfm'):
        img = load_pfm(filepath)    # pfm should be HDR as default
    else:
...
#utilties
def renormalize(normalMap): # normalize the length of each normal to 1
    nOut = np.zeros(normalMap.shape)
    for i in range(0, normalMap.shape[0]):
        normal_1 = (2.0 * normalMap[i,:,:,:] - 1)   # (0,1) -> (-1,1)
        length = np.linalg.norm(normal_1, axis = 2)
        normal_1 = normal_1 / np.dstack((length, length, length))   # (256, 256, 3) / (256, 256, 1)
        nOut[i,:,:,:] = (0.5 * (normal_1 + 1))
    return nOut
